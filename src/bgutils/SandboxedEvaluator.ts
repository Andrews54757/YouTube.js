...
export async function evaluateOnce(evaluator: SandboxedEvaluator, body: string, argNames: string[], argValues: any[], timeoutDuration = 5000): Promise<any> {
...
export function extractFnBodyAndArgs(funcStr: string): { body: string, argNames: string[] } {
...
export function matchArgValues(argNames: string[], argObject: { [key: string]: any }): any[] {
...
export interface SandboxedEvaluator {
  load(): Promise<void>;
  setTimeout(timeoutDuration: number | null): void;
  evaluate(body: string, argNames?: string[], argValues?: any[]): Promise<any>;
  close(): void;
}